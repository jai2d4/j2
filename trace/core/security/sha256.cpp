...
void Sha256::update(const void* data, std::size_t size) {
    ...
    if (bufferSize_ > 0) {
        const std::size_t needed = 64 - bufferSize_;
...
Sha256::Digest Sha256::finalizeBytes() {
    if (!finalized_) {
        const std::uint64_t bitCount = totalBits_;
        ...
        for (int i = 0; i < 8; ++i) {
            lengthBytes[static_cast<std::size_t>(i)] =
                static_cast<std::uint8_t>((bitCount >> (56 - i * 8)) & 0xFFu);
        }
        std::memcpy(buffer_.data() + bufferSize_, lengthBytes.data(), 8);
        compress(buffer_.data());
        bufferSize_ = 0;
        finalized_ = true;
    }
...
std::string Sha256::finalizeHex() { return toHex(finalizeBytes()); }

std::string Sha256::hash(std::string_view text) {
    Sha256 hasher;
    hasher.update(text);
