...
private:
    void feed() {
        ...
        // qsizetype, not int: a buffer larger than 2 GiB is absurd for 200 ms of
        // audio, but truncating the count rather than the buffer is how absurd
        // becomes wrong, so carry the platform's width all the way through.
        qsizetype free = sink_->bytesFree();
        while (free > 0) {
            if (pending_.empty()) {
                if (exhausted_) break;
            ...
            const auto available = static_cast<qsizetype>(pending_.size() - pendingOffset_);
            const qsizetype chunk = std::min(free, available);
            ...
            free -= static_cast<qsizetype>(written);
        }
